...
The gating logic is covered by a smoke test (see [CHANGELOG.md](./CHANGELOG.md#verification)): allowed model bodies are rewritten with the expected sampling, floor, and reasoning values; disallowed model, missing model, and empty allowlist bodies pass through unchanged, byte for byte.
